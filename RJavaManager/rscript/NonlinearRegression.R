...
NonLinearRegression.default <- function(data, depVar, indepVar, statistics = FALSE, model, modelExp = NULL, startValues = NULL,
  ...
  for (i in 1:length(depVar)) {
    ...
    if (confInterval && (class(confOut[[i]]) != "try-error")) {
      ...
      colnames(coefTable) <- c("Parameter", attributes(sumNls[[i]]$coefficient)$dimnames[[2]][1:3], "LL CI*","UL CI*", attributes(sumNls[[i]]$coefficient)$dimnames[[2]][4])
      ...
      colnames(coefTable) <- c("Parameter", attributes(sumNls[[i]]$coefficient)$dimnames[[2]])
      printDataFrame(coefTable)
      cat("\n")
      if (confInterval && (class(confOut[[i]]) == "try-error"))
