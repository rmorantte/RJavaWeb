#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
#     CLUSTER ANALYSIS FUNCTION (DIVISIVE)09.25.2012 Modified: 11.08.13                       	   							  	                                                                              #
#     ClusterDivisive <- function(data, var, idVar, sbinVar, abinVar, ofactorVar, factorVar, stand= TRUE, distance, distMatrix, clusterMem, descriptiveStat, dendrogram, clusterBox, clusterNum)  #
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#

ClusterDivisive <- function(data, var = NULL, idVar =NULL,sbinVar = NULL, abinVar = NULL, ofactorVar = NULL, factorVar = NULL, stand = TRUE, 
                            distance = c("Euclidean", "Maximum", "Manhattan", "Minkowski", "Canberra", "Binary", "Simple Matching","Sokal & Sneath", "Hamann coefficient", "Jaccard", "Dice", "Gower"),
				                    distMatrix = FALSE,copDistance = FALSE, clusterMem = TRUE, descriptiveStat = TRUE, dendrogram = TRUE, clusterBox = TRUE, 
                            clusterNum=2, saveMem = TRUE, outputPath = NULL)
UseMethod("ClusterDivisive") 
  
ClusterDivisive.default <- function(data, var = NULL, idVar =NULL,sbinVar = NULL, abinVar = NULL, ofactorVar = NULL, factorVar = NULL, stand = TRUE, 
                                    distance = c("Euclidean", "Maximum", "Manhattan", "Minkowski", "Canberra", "Binary", "Simple Matching","Sokal & Sneath", "Hamann coefficient", "Jaccard", "Dice", "Gower"),
                                    distMatrix = FALSE, copDistance = FALSE,clusterMem = TRUE, descriptiveStat = TRUE, dendrogram = TRUE, clusterBox = TRUE, 
                                    clusterNum=2, saveMem = TRUE, outputPath = NULL)
  ...
  if((!is.null(var) & !is.null(sbinVar)) & is.null(abinVar) & is.null(factorVar) & is.null(ofactorVar)){
    ...
    z<- c(var,sbinVar)
    if(stand){   
      #     adist <- daisy(x, stand = TRUE, type = list(symm = as.vector(names(tempData[,sbinVar]))))
      adist <- daisy(x, stand = TRUE, type = list(symm = sbinVar))
    }else 
      #       adist <- daisy(x, type = list(symm = as.vector(names(tempData[,sbinVar]))))
  ...
  if (descriptiveStat){
	...
	if (!is.null(outputPath)) {
		if (dendrogram){
			...
			png(paste(outputPath,"DivisiveGraph.png", sep=""),width=2000,height=1000)
			Rdiana <- diana(adist)
			par(cex=0.8,font=3)
			if(!is.null(idVar)){
		  ...
		  plot(Rdiana,which=2,main = "Dendrogram using Divisive Method", hang=-1)
			}
      else plot(Rdiana,which=2,cex=0.6,main = "Dendrogram using Divisive Method", hang=-1)
			if (clusterBox){
  ...
  if(copDistance){write.csv(as.matrix(cophenetic(Rdiana)), row.names = TRUE, file=paste(outputPath, "CopheneticDistances.csv", sep = "")) }
